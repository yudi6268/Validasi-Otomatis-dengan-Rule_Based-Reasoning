...
<?php $__env->startSection('title', 'Form Perjanjian'); ?>

<?php $__env->startSection('back'); ?>
<a href="<?php echo e(auth()->check() && auth()->user()->isWadir() ? route('dashboard.wadir') : route('home', ['section' => 'dashboard'])); ?>" target="_self" style="text-decoration:none; color:#009970; font-size:20px;">
    <i class="fa-solid fa-arrow-left"></i>
</a>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('header_title', 'Form Perjanjian'); ?>
...
<div class="paper">
<form action="<?php echo e(route('perjanjian.store')); ?>" method="POST">
<?php echo csrf_field(); ?>
    ...
    <div class="flex-row">
        ...
        <div class="flex-col">
            <input type="text" class="input-box" name="pihak1_name"
                value="<?php echo e(auth()->user()->nama ?? ''); ?>" readonly>

            <input type="text" class="input-box" name="pihak1_jabatan"
                value="<?php echo e(auth()->user()->jabatan ?? ''); ?>" readonly>
        ...
        <div class="flex-col">
            <input type="text" class="input-box" name="pihak2_name" id="pihak2_name" 
                value="<?php echo e($direktur->nama ?? ''); ?>" readonly tabindex="-1" style="pointer-events:none;background:#e9ecef;">
            ...
            <input type="hidden" name="pihak2_pangkat" value="<?php echo e($direktur->pangkat ?? ''); ?>">
            <input type="hidden" name="pihak2_nip" value="<?php echo e($direktur->nip ?? ''); ?>">
    ...
    <div class="flex-row">
        ...
        <div class="flex-col">
            <input type="date" class="input-box" name="agreement_date" value="<?php echo e(date('Y-m-d')); ?>">
        </div>
    </div>
    ...
    ">
        ...
        <div style="margin-bottom: 12px;">
            ...
            <input
                ...
                value="<?php echo e(isset($jabatanData) && $jabatanData ? $jabatanData->nama_jabatan : ''); ?>"
                readonly
                style="
                    width: 100%;
                    padding: 8px;
        ...
        <div style="margin-bottom: 12px;">
            ...
            <textarea
                ...
                style="
                    ...
                    background: #e9ecef;
                "
            ><?php echo e(isset($jabatanData) && $jabatanData ? $jabatanData->tugas : ''); ?></textarea>
        ...
        <div>
            ...
            <div id="fungsi_container" style="
                ...
                min-height: 60px;
            ">
                <?php if(isset($jabatanData) && $jabatanData && $jabatanData->fungsi): ?>
                    <?php if(is_array($jabatanData->fungsi)): ?>
                        <ol style="margin: 0; padding-left: 20px;">
                            <?php $__currentLoopData = $jabatanData->fungsi; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fungsi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li style="margin-bottom: 4px;"><?php echo e($fungsi); ?></li>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </ol>
                    <?php else: ?>
                        <?php echo e($jabatanData->fungsi); ?>

                    <?php endif; ?>
                <?php endif; ?>
            </div>
            <input type="hidden" name="fungsi_pelaksana" id="fungsi_pelaksana" 
                   value="<?php echo e(isset($jabatanData) && $jabatanData && is_array($jabatanData->fungsi ?? null) ? json_encode($jabatanData->fungsi) : (isset($jabatanData) && $jabatanData ? $jabatanData->fungsi : '')); ?>">
        </div>
    </div>
            ...
            <td>
                <select name="program_nama[]" class="program-nama-dropdown" style="width:95%; font-weight:600; padding:8px;" onchange="handleProgramChange(this)" required>
                    ...
                    <?php $__currentLoopData = $programs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prog): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($prog->nama_program); ?>" data-kode="<?php echo e($prog->kode_program); ?>" data-program-id="<?php echo e($prog->id); ?>"><?php echo e($prog->nama_program); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </td>
            <td><input type="text" name="program_anggaran[]" value="0" readonly style="text-align:right; background:#f0f0f0; cursor:not-allowed;" placeholder="Auto" title="Total otomatis dari kegiatan" /></td>
...
<input type="hidden" id="hierarchical-budget-json" name="hierarchical_budget_json" value="[]">
    ...
    <script>
        window.pihak1Jabatan = <?php echo json_encode(auth()->user()->jabatan, 15, 512) ?>;
    </script>
...
function saveToSupabase(e) {
    ...
    fetch('<?php echo e(route("perjanjian.save")); ?>', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = '<?php echo e(auth()->check() && auth()->user()->isWadir() ? route("dashboard.wadir") : route("home", ["section" => "dashboard"])); ?>';
            }, 1200);
        } else {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Kuliah\Semester 7\Magang\Perjanjian Kinerja\resources\views\perjanjian\create.blade.php ENDPATH**/ ?>