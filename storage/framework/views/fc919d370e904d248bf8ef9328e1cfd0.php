...
<div id="profilePanelContainer" style="display:grid; grid-template-columns:minmax(260px, 320px) minmax(0, 1fr); gap:20px; align-items:start;">
  <input type="hidden" id="profilePanelCsrfToken" value="<?php echo e(csrf_token()); ?>">
  <section style="background:#fff; border-radius:24px; box-shadow:0 18px 40px rgba(0,0,0,0.06); padding:24px; display:grid; gap:18px;">
    <div style="display:grid; justify-items:center; gap:14px; text-align:center;">
      <div style="width:112px; height:112px; border-radius:28px; overflow:hidden; background:#e8f8f3; display:grid; place-items:center; color:#009970; font-size:36px; font-weight:800;">
        <?php if($profilePhotoUrl): ?>
          <img src="<?php echo e($profilePhotoUrl); ?>?v=<?php echo e(time()); ?>" alt="Foto Profil" style="width:100%; height:100%; object-fit:cover;">
        <?php else: ?>
          <?php echo e(strtoupper(substr($dashboardUser->nama ?? 'U', 0, 1))); ?>

        <?php endif; ?>
      </div>
      <div>
        <div style="font-size:24px; font-weight:800; color:#1B2A41;" id="displayNama"><?php echo e($dashboardUser->nama ?? '-'); ?></div>
        <div style="font-size:14px; color:#64748b; margin-top:4px;"><?php echo e($dashboardUser->jabatan ?? 'Pengguna Sistem'); ?></div>
      </div>
    </div>
    ...
    <div id="displayReadOnlySection" style="display:grid; gap:12px; pointer-events:none; cursor:not-allowed; opacity:0.8;">
      <div style="padding:14px 16px; border-radius:16px; background:#f8fcfb;" id="displayEmailBox">
        ...
        <div style="font-size:14px; font-weight:600; color:#243746; word-break:break-word;" id="displayEmail"><?php echo e($dashboardUser->email ?? '-'); ?></div>
      </div>
      <div style="padding:14px 16px; border-radius:16px; background:#f8fcfb;" id="displayNIPBox">
        ...
        <div style="font-size:14px; font-weight:600; color:#243746;" id="displayNIP"><?php echo e($dashboardUser->nip ?? '-'); ?></div>
      </div>
      <div style="padding:14px 16px; border-radius:16px; background:#f8fcfb;" id="displayPangkatBox">
        ...
        <div style="font-size:14px; font-weight:600; color:#243746;" id="displayPangkat"><?php echo e($dashboardUser->pangkat ?? '-'); ?></div>
      </div>
    </div>
  </section>
  ...
  <section style="background:#fff; border-radius:24px; box-shadow:0 18px 40px rgba(0,0,0,0.06); padding:24px; display:grid; gap:18px;">
    <div>
      <h2 style="font-size:28px; font-weight:800; color:#1B2A41; margin-bottom:8px;"><?php echo e($profileTitle); ?></h2>
      <?php if(!$hideDescription): ?>
        <p style="font-size:14px; line-height:1.7; color:#64748b;"><?php echo e($profileDescription); ?></p>
      <?php endif; ?>
    </div>
    ...
    <div style="display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); gap:16px;">
      <div style="padding:18px 20px; border-radius:20px; background:linear-gradient(180deg, #ffffff 0%, #f7fffb 100%); border:1px solid #ebf5f2;">
        ...
        <div style="font-size:18px; font-weight:700; color:#1B2A41;"><?php echo e($dashboardUser->id_pegawai ?? '-'); ?></div>
      </div>
      <div style="padding:18px 20px; border-radius:20px; background:linear-gradient(180deg, #ffffff 0%, #f7fffb 100%); border:1px solid #ebf5f2;">
        ...
        <div style="font-size:18px; font-weight:700; color:#1B2A41; text-transform:capitalize;"><?php echo e($dashboardUser->role ?? 'user'); ?></div>
      </div>
      <div style="padding:18px 20px; border-radius:20px; background:linear-gradient(180deg, #ffffff 0%, #f7fffb 100%); border:1px solid #ebf5f2;">
        ...
        <div style="font-size:18px; font-weight:700; color:#1B2A41; text-transform:capitalize;"><?php echo e($dashboardUser->status ?? 'aktif'); ?></div>
      </div>
      <div style="padding:18px 20px; border-radius:20px; background:linear-gradient(180deg, #ffffff 0%, #f7fffb 100%); border:1px solid #ebf5f2;">
        ...
        <div style="font-size:18px; font-weight:700; color:#1B2A41;"><?php echo e(optional($dashboardUser->updated_at)->format('d M Y') ?? '-'); ?></div>
      </div>
    </div>
    ...
    <div style="padding:16px; border:1px dashed #d6ebe4; border-radius:16px; background:#f8fcfb;">
      ...
      <div style="min-height:110px; border-radius:12px; background:#fff; border:1px solid #e4efeb; display:grid; place-items:center; padding:10px;">
        <?php if($signatureUrl): ?>
          <img id="currentSignatureImg" src="<?php echo e($signatureUrl); ?>?v=<?php echo e(time()); ?>" alt="Tanda Tangan" style="max-width:100%; max-height:90px; object-fit:contain;">
    ...
    <div style="display:grid; grid-template-columns:minmax(0, 1fr) 220px; gap:18px; align-items:start;">
      ...
      <div style="padding:20px; border-radius:20px; background:#f8fcfb; border:1px dashed #d6ebe4; display:grid; gap:10px; justify-items:center; text-align:center; min-height:180px;">
        ...
        <div style="width:100%; min-height:110px; border-radius:16px; background:#fff; border:1px solid #e4efeb; display:grid; place-items:center; padding:10px;">
          <?php if($signatureUrl): ?>
            <img src="<?php echo e($signatureUrl); ?>?v=<?php echo e(time()); ?>" alt="Tanda Tangan" style="max-width:100%; max-height:90px; object-fit:contain;">
          <?php else: ?>
  ...
  <div style="width:min(760px, 100%); max-height:90vh; overflow:auto; background:#fff; border-radius:16px; box-shadow:0 30px 80px rgba(0,0,0,0.24);">
    ...
    <div style="padding:16px 18px; display:grid; gap:12px;">
      <div>
        ...
        <input type="text" id="modalEditNama" value="<?php echo e($dashboardUser->nama ?? ''); ?>" style="width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:14px;">
      </div>
      <div>
        ...
        <input type="email" id="modalEditEmail" value="<?php echo e($dashboardUser->email ?? ''); ?>" style="width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:14px;">
      </div>
      <div>
        ...
        <input type="text" id="modalEditNIP" value="<?php echo e($dashboardUser->nip ?? ''); ?>" style="width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:14px;">
      </div>
      <div>
        ...
        <input type="text" id="modalEditPangkat" value="<?php echo e($dashboardUser->pangkat ?? ''); ?>" style="width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:14px;">
      </div>
...
<script>
  ...
  if (saveProfileBtn) {
    saveProfileBtn.addEventListener('click', async function() {
      ...
      try {
        const response = await fetch('<?php echo e(route("profil.update")); ?>', {
          method: 'POST',
          headers: {
...
<?php endif; ?><?php /**PATH E:\Kuliah\Semester 7\Magang\Perjanjian Kinerja\resources\views/dashboard/partials/profile-panel.blade.php ENDPATH**/ ?>