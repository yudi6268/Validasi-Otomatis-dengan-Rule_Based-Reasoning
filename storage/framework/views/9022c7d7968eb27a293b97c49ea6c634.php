...
<body style="background:#e6fcfc;min-height:100vh;">
    ...
    <?php if(!$isDirektur): ?>
        <div class="user-header-fixed">
            ...
            <?php if(isset($perjanjian->rejected) && $perjanjian->rejected): ?>
    ...
    <?php if($isDirektur && $status === 'menunggu'): ?>
        <style>
            .aksi-container {
                position: fixed;
                top: 88px;
        ...
        <div class="aksi-container">
            <button id="btn-setujui" type="button" class="aksi-btn terima"
                onclick="<?php if(!empty($approvedOther)): ?>document.getElementById('modal-sudah-disetujui').style.display='flex'<?php else: ?> document.getElementById('modal-setujui').style.display='flex'<?php endif; ?>">
        ...
        <div id="modal-setujui"
            ...
            <div
                ...
                <p style="text-align:center; font-size:15px; margin-bottom:22px;">Apa anda yakin untuk menyetujui perjanjian
                    ini?</p>
                <form id="form-setujui" action="<?php echo e(route('direktur.perjanjian.approve', $perjanjian->id)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
        ...
        <div id="modal-tolak"
            ...
            <div
                ...
                <form id="form-tolak" action="<?php echo e(route('direktur.perjanjian.reject', $perjanjian->id)); ?>" method="POST"
                    autocomplete="off">
                    <?php echo csrf_field(); ?>
                    <input type="text" name="nama" value="<?php echo e($perjanjian->pihak2_name ?? ''); ?>" readonly
                        placeholder="Nama Lengkap"
                        style="width:100%;margin-bottom:10px;padding:8px 10px;border:1px solid #bbb;border-radius:6px;font-size:15px;">
                    <input type="text" name="jabatan" value="<?php echo e($perjanjian->pihak2_jabatan ?? ''); ?>" readonly
                        placeholder="Jabatan"
                        style="width:100%;margin-bottom:10px;padding:8px 10px;border:1px solid #bbb;border-radius:6px;font-size:15px;">
                    <input type="text" name="tanggal"
                        value="<?php echo e(\Carbon\Carbon::parse($perjanjian->agreement_date ?? $perjanjian->created_at)->translatedFormat('d-m-Y')); ?>"
            ...
            if (formTolak) {
                formTolak.onsubmit = function (e) {
                    ...
                    fetch(formTolak.action, {
                        ...
                        .then(res => res.json())
                        .then(data => {
                            try { if (window.opener && window.opener.postMessage) { window.opener.postMessage({ type: 'PERJANJIAN_STATUS_CHANGED', id: <?php echo e((int) $perjanjian->id); ?> }, window.location.origin); } } catch (e) { }
        ...
        <div id="modal-sudah-disetujui"
            ...
            <div style="background:#fff; border-radius:14px; padding:36px 28px; max-width:440px; width:95vw; box-shadow:0 6px 32px rgba(0,0,0,0.18); position:relative; text-align:center;">
                ...
                <p style="font-size:14px; color:#555; line-height:1.6; margin-bottom:22px;">
                    Sudah terdapat Perjanjian Kinerja atas nama <strong><?php echo e($perjanjian->pihak1_name); ?></strong>
                    yang telah disetujui sebelumnya
                    (disetujui pada <strong><?php echo e(optional($approvedOther->updated_at)->translatedFormat('d F Y')); ?></strong>).
                    <br><br>
    ...
    <div class="preview-center-wrapper">
                    ...
                    <div class="modal-content"
                        ...
                        <div style="background:#f8f9fa;padding:20px 24px;border-bottom:1px solid #dee2e6;">
                            ...
                            <div style="margin-bottom:8px;display:flex;align-items:flex-start;">
                                ...
                                <span style="font-weight:600;color:#333;margin:0 8px;flex-shrink:0;">:</span>
                                <span
                                    style="color:#333;flex:1;word-wrap:break-word;"><?php echo e($perjanjian->pihak2_name ?? 'dr. ARMA ROOSALINA, M.Kes'); ?></span>
                            </div>
                            <div style="margin-bottom:8px;display:flex;align-items:flex-start;">
                                <span
                                ...
                                <span style="font-weight:600;color:#333;margin:0 8px;flex-shrink:0;">:</span>
                                <span
                                    style="color:#333;flex:1;word-wrap:break-word;"><?php echo e($perjanjian->pihak2_jabatan ?? 'Direktur UOBK RSUD Bangil'); ?></span>
                            </div>
                            <div style="display:flex;align-items:flex-start;">
                                ...
                                <span style="font-weight:600;color:#333;margin:0 8px;flex-shrink:0;">:</span>
                                <span
                                    style="color:#333;flex:1;word-wrap:break-word;"><?php echo e(\Carbon\Carbon::parse($perjanjian->agreement_date ?? $perjanjian->created_at)->translatedFormat('d F Y')); ?></span>
                            </div>
                        </div>
                        ...
                        <div style="padding:24px;">
                                ...
                                <div style="display:flex;align-items:flex-start;gap:12px;">
                                    ...
                                    <div style="flex:1;">
                                        ...
                                        <div style="color:#333;line-height:1.6;font-size:16px;font-weight:500;">
                                            <?php echo e($perjanjian->rejection_reason ?? 'Target kinerja tidak sesuai dari perjanjian sebelumnya. harap revisi bagian target di indikator kinerja individu.'); ?>
                            ...
                            <div style="text-align:center;">
                                <a href="<?php echo e(route('perjanjian.edit', ['id' => $perjanjian->id, 'from' => request('from')])); ?>"
        ...
        <?php if($isDirektur): ?>
            <?php if($status === 'ditolak' || $perjanjian->rejected): ?>
                <div class="status-badge" style="background:#DC3545;color:#fff;">
                    <?php if(empty($for_pdf) || !$for_pdf): ?>
                        <i class="fa-solid fa-circle-xmark"></i>
                    <?php else: ?>
            ...
            <?php elseif($status === 'disetujui' || !empty($perjanjian->pihak2_signature)): ?>
                <div class="status-badge" style="background:#28a745;color:#fff;">
                    <?php if(empty($for_pdf) || !$for_pdf): ?>
                        <i class="fa-solid fa-circle-check"></i>
                    <?php else: ?>
        ...
        <div class="preview-card">
            ...
            <div class="page" style="font-size: 11pt;">
                <div class="header" style="text-align:center; margin-bottom:0;">
                    <?php if(!empty($for_pdf) && !empty($logo_data)): ?>
                        <img src="<?php echo e($logo_data); ?>" class="logo" alt="Logo Pemda"
                            style="max-height:50px;width:auto;display:inline-block;margin-bottom:2px;">
                    <?php else: ?>
                        <img src="<?php echo e($logoPemda ?? asset('images/logo_pemda.png')); ?>" class="logo" alt="Logo Pemda"
                    ...
                    <div style="font-weight:bold;font-size:12pt;margin-bottom:1px;">PERJANJIAN KINERJA TAHUN <?php echo e($tahun ?? '2025'); ?></div>
                ...
                <div class="content-section" style="text-align:justify; line-height:1.3; width: 90%; !important">
                    ...
                    <div style="margin-bottom:4px; text-align:left;">
                        ...
                        <?php echo e($perjanjian->pihak1_name ?? '-'); ?>

                    </div>
                    <div style="margin-bottom:4px; text-align:left;">
                        <span style="display:inline-block;width:100px;">Jabatan</span>:
                        <?php echo e($perjanjian->pihak1_jabatan ?? '-'); ?>

                    </div>
                    <div style="margin-bottom:4px;">Selanjutnya disebut pihak pertama.</div>
                    <div style="margin-bottom:4px; text-align:left;">
                        <span style="display:inline-block;width:100px;">Nama</span>:
                        <?php echo e($perjanjian->pihak2_name ?? '-'); ?>

                    </div>
                    <div style="margin-bottom:4px; text-align:left;">
                        <span style="display:inline-block;width:100px;">Jabatan</span>:
                        <?php echo e($perjanjian->pihak2_jabatan ?? '-'); ?>
                    ...
                    <div class="sig-flex-row" style="display:flex;justify-content:space-between; margin-top:5px;">
                        <div class="sig-flex-col" style="text-align:center;width:45%;">
                            PIHAK KEDUA<br><br>
                            <?php if(!empty($for_pdf) && !empty($pihak2_ttd_data)): ?>
                                <img src="<?php echo e($pihak2_ttd_data); ?>" style="max-height:50px;margin:2px 0;" alt="TTD Pihak 2">
                            <?php elseif(!empty($perjanjian->pihak2_signature)): ?>
                                <img src="<?php echo e($perjanjian->pihak2_signature); ?>" style="max-height:50px;margin:2px 0;"
                                    alt="TTD Pihak 2" loading="lazy">
                            <?php elseif(!empty($perjanjian->pihak2_ttd_path)): ?>
                                <img src="<?php echo e(asset('storage/' . $perjanjian->pihak2_ttd_path)); ?>"
                                    style="max-height:50px;margin:2px 0;" alt="TTD Pihak 2" loading="lazy">
                            <?php else: ?>
                                <div style="height:50px;"></div>
                            <?php endif; ?>
                            <br>
                            <u><?php echo e($perjanjian->pihak2_name ?? '-'); ?></u><br>
                            <?php echo e($perjanjian->pihak2_pangkat ?? '-'); ?><br>
                            NIP. <?php echo e($perjanjian->pihak2_nip ?? '-'); ?>

                        </div>
                        <div class="sig-flex-col" style="text-align:center;width:45%;">
                            Pasuruan,
                            <?php echo e(Carbon\Carbon::parse($perjanjian->agreement_date ?? $perjanjian->created_at)->translatedFormat('d F Y')); ?><br>
                            PIHAK PERTAMA<br>
                            <?php if(!empty($for_pdf) && !empty($pihak1_ttd_data)): ?>
                                <img src="<?php echo e($pihak1_ttd_data); ?>" style="max-height:50px;margin:2px 0;" alt="TTD Pihak 1">
                            <?php elseif(!empty($perjanjian->pihak1_ttd)): ?>
                                <img src="<?php echo e($perjanjian->pihak1_ttd); ?>" style="max-height:50px;margin:2px 0;"
                                    alt="TTD Pihak 1" loading="lazy">
                            <?php else: ?>
                                <div style="height:50px;"></div>
                            <?php endif; ?>
                            <br>
                            <u><?php echo e($perjanjian->pihak1_name ?? '-'); ?></u><br>
                            <?php echo e($perjanjian->pihak1_pangkat ?? '-'); ?><br>
                            NIP. <?php echo e($perjanjian->pihak1_nip ?? '-'); ?>
            ...
            <?php if($hasTabelAData || $hasProgramData): ?>
                ...
                <div class="page">
                    <div class="header" style="text-align:center;">
                        ...
                        <div style="font-weight:bold;font-size:1.1rem;margin-bottom:2px;">TAHUN <?php echo e($tahun ?? '2025'); ?></div>
                    </div>
                        ...
                        <table
                            ...
                            <tr style="border:0 !important;">
                                ...
                                <td style="padding:6px 0;vertical-align:top;line-height:1.15;border:none !important;">
                                    ...
                                    ?>
                                    <?php if(!empty($fungsiItems)): ?>
                                        <ol style="margin:0;padding-left:18px;list-style-type:lower-alpha;line-height:1.15;">
                                            <?php $__currentLoopData = $fungsiItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <?php
                                                    // Remove any existing alphabetical prefix like "a. " to avoid duplication
                                                    $cleanFi = preg_replace('/^[a-zA-Z]\.\s*/', '', trim((string) $fi));
                                                ?>
                                                <li style="margin-bottom:3px;text-align:justify;"><?php echo e($cleanFi); ?></li>
                        ...
                        <div class="table-responsive">
                            <table class="table table-fixed" style="table-layout: fixed; width: 90%;">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                    <?php $__empty_1 = true; $__currentLoopData = $sasaranData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <tr>
                                            <td style="text-align:center;"><?php echo e((int) $i + 1); ?></td>
                                            <td><?php echo e($row['sasaran'] ?? ''); ?></td>
                                            <td><?php echo e($row['indikator'] ?? ''); ?></td>
                                            <td style="text-align:center;"><?php echo e($row['satuan'] ?? ''); ?></td>
                                            <td style="text-align:center;"><?php echo e($row['target'] ?? ''); ?></td>
                                        </tr>
                            ...
                            <div class="table-responsive">
                                <table class="table table-fixed" style="table-layout: fixed; width: 90%;">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                        <?php $__empty_1 = true; $__currentLoopData = $programData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                            <tr>
                                                <td style="text-align:center;"><?php echo e($row['no'] ?? ($i + 1)); ?></td>
                                                <td><?php echo e($row['name'] ?? ''); ?></td>
                                                <td style="text-align:right;"><?php echo e(number_format((int) $row['amount'], 0, ',', '.')); ?>

                                                </td>
                                                <td style="font-size:11px; word-wrap:break-word; white-space:normal;">
                                                    <?php echo e($row['source'] ?? '-'); ?>
                                    ...
                                    <?php if(($programTotal ?? 0) > 0): ?>
                                        <tfoot>
                                            <tr>
                                                <td colspan="2" style="text-align:right;font-weight:700;">Total</td>
                                                <td style="text-align:right;font-weight:700;">
                                                    <?php echo e(number_format((int) $programTotal, 0, ',', '.')); ?>
                        ...
                        <div style="margin-top:10px;">
                            ...
                            <div class="sig-flex-row" style="display:flex;justify-content:space-between;">
                                <div class="sig-flex-col" style="text-align:center;width:45%;">
                                    PIHAK KEDUA<br><br>
                                    <?php if(!empty($for_pdf) && !empty($pihak2_ttd_data)): ?>
                                        <img src="<?php echo e($pihak2_ttd_data); ?>" style="max-height:60px;margin:5px 0;"
                                            alt="TTD Pihak 2">
                                    <?php elseif(!empty($perjanjian->pihak2_signature)): ?>
                                        <img src="<?php echo e($perjanjian->pihak2_signature); ?>" style="max-height:60px;margin:5px 0;"
                                            alt="TTD Pihak 2" loading="lazy">
                                    <?php elseif(!empty($perjanjian->pihak2_ttd_path)): ?>
                                        <img src="<?php echo e(asset('storage/' . $perjanjian->pihak2_ttd_path)); ?>"
                                ...
                                <div class="sig-flex-col" style="text-align:center;width:45%;">
                                    ...
                                    PIHAK PERTAMA<br>
                                    <?php if(!empty($for_pdf) && !empty($pihak1_ttd_data)): ?>
                                        <img src="<?php echo e($pihak1_ttd_data); ?>" style="max-height:60px;margin:5px 0;"
                                            alt="TTD Pihak 1">
                                    <?php elseif(!empty($perjanjian->pihak1_ttd)): ?>
                                        <img src="<?php echo e($perjanjian->pihak1_ttd); ?>" style="max-height:60px;margin:5px 0;"
                                            alt="TTD Pihak 1" loading="lazy">
                                    <?php else: ?>
                                        <div style="height:60px;"></div>
                                    <?php endif; ?>
            ...
            <?php if($hasTabelBData || $hasTabelDData): ?>
                <!-- PAGE 3: RENCANA AKSI -->
                            ...
                            <div class="table-responsive">
                                <table class="table table-fixed" style="table-layout: fixed; width: 100%;">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                        <?php $__empty_1 = true; $__currentLoopData = $rencanaAksiData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                            <tr>
                                                <td style="text-align:center;"><?php echo e((int) $i + 1); ?></td>
                                                <td><?php echo e($row['sasaran'] ?? ''); ?></td>
                                                <td><?php echo e($row['indikator'] ?? ''); ?></td>
                                                <td style="text-align:center;"><?php echo e($row['target'] ?? ''); ?></td>
                                                <td style="text-align:right;"><?php echo e(!empty($row['tw1']) ? $row['tw1'] : ''); ?></td>
                                                <td style="text-align:right;"><?php echo e(!empty($row['tw2']) ? $row['tw2'] : ''); ?></td>
                                                <td style="text-align:right;"><?php echo e(!empty($row['tw3']) ? $row['tw3'] : ''); ?></td>
                                                <td style="text-align:right;"><?php echo e(!empty($row['tw4']) ? $row['tw4'] : ''); ?></td>
                                            </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                            <tr>
                            ...
                            ?>
                                <div class="table-responsive" style="margin-top:18px;">
                                    <table class="table table-fixed" style="table-layout: fixed; width: 100%;">
                                        ...
                                        <tbody>
                                            <?php $__empty_1 = true; $__currentLoopData = $programs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                                <tr>
                                                    <td style="text-align:center;"><?php echo e($row['no'] ?? ($i + 1)); ?></td>
                                                    <td style="font-weight: <?php echo e(($row['level'] ?? '') === 'program' ? 'bold' : 'normal'); ?>; font-style: <?php echo e(($row['level'] ?? '') === 'kegiatan' ? 'italic' : 'normal'); ?>;"><?php echo e($row['name']); ?></td>
                                                    <td style="text-align:right;"><?php echo e(number_format((int) $row['amount'], 0, ',', '.')); ?></td>
                                                    <td style="text-align:right; background-color: <?php echo e(empty($row['tw1']) ? '#f9f9f9' : 'transparent'); ?>;"><?php echo e(!empty($row['tw1']) ? number_format((int) preg_replace('/[^0-9]/', '', (string) $row['tw1']), 0, ',', '.') : ''); ?></td>
                                                    <td style="text-align:right; background-color: <?php echo e(empty($row['tw2']) ? '#f9f9f9' : 'transparent'); ?>;"><?php echo e(!empty($row['tw2']) ? number_format((int) preg_replace('/[^0-9]/', '', (string) $row['tw2']), 0, ',', '.') : ''); ?></td>
                                                    <td style="text-align:right; background-color: <?php echo e(empty($row['tw3']) ? '#f9f9f9' : 'transparent'); ?>;"><?php echo e(!empty($row['tw3']) ? number_format((int) preg_replace('/[^0-9]/', '', (string) $row['tw3']), 0, ',', '.') : ''); ?></td>
                                                    <td style="text-align:right; background-color: <?php echo e(empty($row['tw4']) ? '#f9f9f9' : 'transparent'); ?>;"><?php echo e(!empty($row['tw4']) ? number_format((int) preg_replace('/[^0-9]/', '', (string) $row['tw4']), 0, ',', '.') : ''); ?></td>
                                                </tr>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                        ...
                                        <?php if(($hierarchicalBudgetTotal ?? 0) > 0): ?>
                                            <tfoot>
                                                <tr>
                                                    <td colspan="2" style="text-align:right;font-weight:700;">Total</td>
                                                    <td style="text-align:right;font-weight:700;"><?php echo e(number_format((int) $hierarchicalBudgetTotal, 0, ',', '.')); ?></td>
                                                    <td colspan="4"></td>
                                                </tr>
                                            </tfoot>
                                        <?php endif; ?>
                        ...
                        <?php endif; ?>
                            ...
                            <div class="signature-wrapper" style="margin-top: 20px;">
                                <div class="signature-block">
                                    Pasuruan, <?php echo e(Carbon\Carbon::parse($perjanjian->agreement_date ?? $perjanjian->created_at)->translatedFormat('d F Y')); ?><br>
                                    <?php echo e(strtoupper($perjanjian->pihak1_jabatan ?? '-')); ?><br><br>

                                    <?php if(!empty($for_pdf) && !empty($pihak1_ttd_data)): ?>
                                        <img src="<?php echo e($pihak1_ttd_data); ?>" style="max-height:60px;margin:5px 0;" alt="TTD Pihak 1">
                                    <?php elseif(!empty($perjanjian->pihak1_ttd)): ?>
                                        <img src="<?php echo e($perjanjian->pihak1_ttd); ?>" style="max-height:60px;margin:5px 0;" alt="TTD Pihak 1" loading="lazy">
                                    <?php else: ?>
                                        <div style="height:60px;"></div>
                                    <?php endif; ?>
                                    <br>
    ...
    <?php if(!$isDirektur): ?>
        <div class="footer-fixed">
            © <?php echo e(date('Y')); ?> © 2026 RSUD Bangil | Validasi Otomatis Laporan Kinerja RSUD Bangil
        </div>
    <?php endif; ?>
...
</html><?php /**PATH E:\Kuliah\Semester 7\Magang\Perjanjian Kinerja\resources\views/perjanjian/print.blade.php ENDPATH**/ ?>