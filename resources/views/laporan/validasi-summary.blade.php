...
<body>
  ...
  <div class="layout">
    @php $isWadir = auth()->user()?->isWadir(); @endphp
    @if($isWadir)
    ...
    <main class="main-content">
      ...
      <div class="info-card">
        ...
        <div class="info-grid">
          <div class="info-item"><div class="info-label">Nama Pegawai</div><div class="info-value">{{ $perjanjian->pihak1_name ?? '-' }}</div></div>
          <div class="info-item"><div class="info-label">NIP</div><div class="info-value">{{ $perjanjian->pihak1_nip ?? '-' }}</div></div>
          <div class="info-item"><div class="info-label">Jabatan</div><div class="info-value">{{ $perjanjian->pihak1_jabatan ?? '-' }}</div></div>
          <div class="info-item"><div class="info-label">Tahun</div><div class="info-value">{{ $perjanjian->tahun ?? date('Y') }}</div></div>
          <div class="info-item"><div class="info-label">Atasan Langsung</div><div class="info-value">{{ $perjanjian->pihak2_name ?? '-' }}</div></div>
        </div>
      </div>
      ...
      <div class="tw-banner tw-banner-{{ $tw }}">
        <div class="tw-banner-icon">{{ $tw }}</div>
        <div class="tw-banner-info">
          <div class="tw-banner-title">Triwulan {{ $tw }} — Aktif</div>
          <div class="tw-banner-sub">{{ $twPeriods[$tw] ?? '' }}</div>
        </div>
      </div>
      ...
      <div class="stat-grid">
        ...
        <div class="stat-card stat-invalid">
          ...
          <div class="stat-num">{{ $stats['tidak_valid'] }}</div>
      ...
      @if(!$res)
        <div class="alert-info">
          <i class="fas fa-info-circle"></i>
          Laporan untuk Triwulan {{ $tw }} belum tersedia. Isi laporan realisasi terlebih dahulu.
        </div>
      @else
        <div class="score-section">
          <div class="score-row">
            <div class="score-big {{ $scoreCls }}">{{ $score }}</div>
            <div class="score-meta">
              <div class="score-lbl">Skor Validasi Triwulan {{ $tw }}</div>
              <div class="score-bar-wrap">
                <div class="score-bar-fill {{ $barCls }}" style="width:{{ $score }}%;"></div>
              </div>
            </div>
            @if($res['is_valid'])
          ...
          @if(!empty($res['issues']))
            <div class="finding-group">
              <div class="finding-group-title issues"><i class="fas fa-circle-xmark"></i> Tidak Valid ({{ count($res['issues']) }})</div>
              <div class="finding-list">
                @foreach($res['issues'] as $item)
                  <div class="finding-item issue">
                    <div class="finding-item-msg">{{ $item['message'] }}</div>
                    @if(!empty($item['fix']))<div class="finding-item-fix"><i class="fas fa-wrench"></i> {{ $item['fix'] }}</div>@endif
                  </div>
                @endforeach
              </div>
            </div>
          ...
          @if(!empty($res['warnings']))
            <div class="finding-group">
              <div class="finding-group-title warnings"><i class="fas fa-triangle-exclamation"></i> Peringatan ({{ count($res['warnings']) }})</div>
              <div class="finding-list">
                @foreach($res['warnings'] as $item)
          ...
          @if(!empty($res['suggestions']))
            <div class="finding-group">
              <div class="finding-group-title suggestions"><i class="fas fa-pen-to-square"></i> Revisi / Saran ({{ count($res['suggestions']) }})</div>
              <div class="finding-list">
                @foreach($res['suggestions'] as $item)
                  <div class="finding-item suggestion"><div class="finding-item-msg">{{ $item['message'] }}</div></div>
                @endforeach
              </div>
            </div>
          @endif

          @if(empty($res['issues']) && empty($res['warnings']) && empty($res['suggestions']))
      ...
      <div class="confirm-section">
        ...
        @elseif($stats['tidak_valid'] === 0 && $stats['peringatan'] === 0)
          <p>Laporan Triwulan {{ $tw }} telah lolos validasi otomatis. Klik tombol di bawah untuk mengkonfirmasi laporan sudah sesuai.</p>
          <button type="button" class="btn-confirm" onclick="saveValidationAndRedirect({{ $tw }})"><i class="fas fa-check-double"></i> Konfirmasi</button>
        @elseif($stats['tidak_valid'] === 0)
          <p>Laporan lolos validasi namun terdapat <strong>{{ $stats['peringatan'] }} peringatan</strong> dan <strong>{{ $stats['revisi'] }} saran</strong>. Anda tetap dapat melanjutkan atau melakukan perbaikan.</p>
          <button type="button" class="btn-confirm" onclick="saveValidationAndRedirect({{ $tw }})"><i class="fas fa-check"></i> Konfirmasi</button>
        @else
          <p>Terdapat <strong>{{ $stats['tidak_valid'] }} masalah kritis</strong> yang harus diperbaiki sebelum laporan dapat dikonfirmasi.</p>
          <a href="{{ route('laporan.kinerja') }}" class="btn-confirm" style="background:linear-gradient(135deg,#e53935,#c62828);"><i class="fas fa-pen"></i> Perbaiki Laporan</a>
        @endif
      </div>
  ...
  <script>
    function saveValidationAndRedirect(tw) {
      ...
      const validationData = {
        ...
        total: {{ $stats['total'] }},
        valid: {{ $stats['valid'] }},
        invalid: {{ $stats['tidak_valid'] }},
        warnings: {{ $stats['peringatan'] }},
        suggestions: {{ $stats['revisi'] }},
        perjanjianId: {{ $perjanjian?->id ?? 'null' }},
      ...
      fetch('/api/validasi-laporan', {
        ...
        .then(data => {
          if (data.success) {
            ...
            window.location.href = '{{ route("dashboard.wadir", ["panel" => "validasi"]) }}';
          } else {
