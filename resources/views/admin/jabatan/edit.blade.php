...
<div class="row">
    <div class="col-md-8">
        <div class="data-table">
            ...
            @endif
            <form id="jabatanForm" method="POST" action="{{ route('admin.jabatan.update', $jabatan['id']) }}" autocomplete="off">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label class="form-label">Nama Jabatan <span class="text-danger">*</span></label>
                    <input type="text" name="nama_jabatan" class="form-control" placeholder="Contoh: Direktur" value="{{ old('nama_jabatan', $jabatan['nama_jabatan']) }}" required @if(session('success')) disabled @endif>
                </div>
                <div class="mb-3">
                    <label class="form-label">Tugas</label>
                    <textarea name="tugas" class="form-control" placeholder="Deskripsi tugas jabatan ini..." rows="3" @if(session('success')) disabled @endif>{{ old('tugas', $jabatan['tugas']) }}</textarea>
                </div>
                <label class="form-label">Fungsi</label>
                <div id="fungsiList">
                    @php
                        $fungsiList = old('fungsi', $jabatan['fungsi'] ?? []);
                ...
                <div id="membawahiList">
                    @php
                        $membawahi = old('membawahi', $jabatan['membawahi'] ?? []);
                    ...
                    @endphp
                    @foreach($membawahi as $index => $unit)
                ...
                <div class="form-check mb-2">
                    <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActive" {{ old('is_active', $jabatan['is_active']) ? 'checked' : '' }} @if(session('success')) disabled @endif>
