            ...
            <form action="{{ route('admin.templates.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama_template" class="form-control @error('nama_template') is-invalid @enderror" 
                           value="{{ old('nama_template') }}" placeholder="Contoh: Template Perjanjian Kinerja Tahunan" required>
                    @error('nama_template')
                ...
                <div class="mb-3">
                    ...
                    <select name="tipe" class="form-select @error('tipe') is-invalid @enderror" required>
                        ...
                        <option value="perjanjian" {{ old('tipe') == 'perjanjian' ? 'selected' : '' }}>Perjanjian Kinerja</option>
                        <option value="laporan" {{ old('tipe') == 'laporan' ? 'selected' : '' }}>Laporan Kinerja</option>
                    </select>
                    @error('tipe')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea name="konten" class="form-control @error('konten') is-invalid @enderror" 
                              rows="12" required placeholder="Masukkan konten template (bisa berisi HTML)...">{{ old('konten') }}</textarea>
                    @error('konten')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror" 
                              rows="3" placeholder="Catatan atau keterangan tambahan tentang template ini">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
