{{-- Preview khusus direktur/pimpinan --}}
@if($status === 'ditolak')
    {{-- Tidak ada header/bar merah di sini, hanya konten utama jika diperlukan --}}
@elseif($status === 'disetujui')
                ...
                <div style="display: flex; gap: 48px; margin-top: 32px; justify-content: center;">
                        <div class="ttd-pihak1" style="text-align:center;">
                                ...
                                @if(!empty($perjanjian->pihak1_signature))
                                        <img src="{{ $perjanjian->pihak1_signature }}" alt="Tanda Tangan Pihak Pertama" style="height:64px; margin:8px 0;">
                                @else
                                        <div style="height:64px;">(Belum ada tanda tangan)</div>
                                @endif
                                <div style="margin-top:8px;">{{ $perjanjian->pihak1_name ?? '-' }}</div>
                        </div>
                        <div class="ttd-pihak2" style="text-align:center;">
                                ...
                                @if(!empty($perjanjian->pihak2_signature))
                                        <img src="{{ $perjanjian->pihak2_signature }}" alt="Tanda Tangan Pihak Kedua" style="height:64px; margin:8px 0;">
                                @else
                                        <div style="height:64px;">(Belum ada tanda tangan)</div>
                                @endif
                                <div style="margin-top:8px;">{{ $perjanjian->pihak2_name ?? '-' }}</div>
                        </div>
                </div>
@elseif($status === 'menunggu')
        ...
        <div style="position: fixed; top: 32px; right: 32px; z-index: 9999; display: flex; gap: 16px;">
                <form method="POST" action="/perjanjian/{{ $perjanjian->id }}/setujui">
                        @csrf
        ...
        <div id="modalTolak" class="modal-bg" style="display:none;">
                <div class="modal-box">
                        ...
                        <form method="POST" action="/perjanjian/{{ $perjanjian->id }}/tolak" id="formTolak" onsubmit="event.preventDefault(); document.getElementById('modalSuccess').style.display='flex'; setTimeout(function(){document.getElementById('modalTolak').style.display='none'; document.getElementById('modalSuccess').style.display='none'; document.getElementById('formTolak').submit();}, 1200);">
                                @csrf
                                <div class="modal-form-group">
                                        <input type="text" class="modal-form-input" value="{{ auth()->user()->nama ?? '' }}" placeholder="Nama Lengkap" readonly>
                                </div>
                                <div class="modal-form-group">
                                        <input type="text" class="modal-form-input" value="{{ auth()->user()->jabatan ?? '' }}" placeholder="Jabatan" readonly>
                                </div>
                                <div class="modal-form-group">
                                        <input type="text" class="modal-form-input" value="{{ date('d-m-Y') }}" placeholder="Tanggal" readonly>
                                </div>
                                <div class="modal-form-group">
