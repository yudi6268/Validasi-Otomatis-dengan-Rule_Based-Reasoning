        ...
        @forelse($perjanjians as $perjanjian)
            ...
            <div class="perjanjian-card">
                ...
                <div class="card-content">
                    <div class="card-title">Perjanjian Kinerja {{ date('Y', strtotime($perjanjian->created_at)) }}</div>
                    <div class="card-status status-{{ $status }}">{{ $statusText }}</div>
                    <div class="card-date">{{ $perjanjian->created_at->format('d F Y') }}</div>
                </div>
                
                <div class="card-actions">
                    <!-- View Button -->
                    <a href="{{ route('perjanjian.show', $perjanjian->id) }}" class="action-btn btn-view" title="Lihat">
                        <i class="fas fa-eye"></i>
                    </a>
                    
                    @if($status === 'menunggu')
                        <!-- Edit Button (Menunggu) -->
                        <a href="{{ route('perjanjian.edit', $perjanjian->id) }}" class="action-btn btn-edit" title="Edit">
                            <i class="fas fa-pencil-alt"></i>
                        </a>
                        <!-- Delete Button (Menunggu) -->
                        <button onclick="confirmDelete({{ $perjanjian->id }})" class="action-btn btn-delete" title="Hapus">
                            <i class="fas fa-trash-alt"></i>
                        </button>
                    @elseif($status === 'ditolak')
                        <!-- Alasan Button (Ditolak) -->
                        <button onclick="showReason('{{ addslashes($perjanjian->rejection_reason ?? 'Tidak ada alasan') }}')" class="btn-reason" title="Alasan">
                            Alasan
                        </button>
                    @elseif($status === 'disetujui')
                        <!-- Download Button (Disetujui) -->
                        <a href="{{ route('perjanjian.download', $perjanjian->id) }}" class="action-btn btn-download" title="Download">
                            <i class="fas fa-download"></i>
                        </a>
                    @endif
                    
                    <!-- Delete Button (Terkirim & Disetujui) -->
                    @if($status === 'terkirim' || $status === 'disetujui')
                        <button onclick="confirmDelete({{ $perjanjian->id }})" class="action-btn btn-delete" title="Hapus">
                            <i class="fas fa-trash-alt"></i>
                        </button>
                    @endif
...
<!-- Floating Add Button -->
<button class="btn-add" onclick="window.location='{{ route('perjanjian.create') }}'" title="Tambah Perjanjian Baru">
...
@endsection
        ...
        <div class="stat-card" data-filter="all">
            <div class="stat-number" style="color: #009970;">{{ ($counts['approved'] ?? 0) + ($counts['rejected'] ?? 0) + ($counts['waiting'] ?? 0) }}</div>
...
function renderRows(items, filter) {
    ...
    items.forEach((it, idx) => {
        ...
        const editUrl = `{{ url('perjanjian') }}/${it.id}/edit`;
