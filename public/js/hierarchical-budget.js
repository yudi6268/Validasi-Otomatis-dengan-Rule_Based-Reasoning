        ...
        program.kegiatan.forEach((kegiatan, kIdx) => {
            ...
            const kegiatanRow = createBudgetRow({
                ...
                inputNames: {
                    ...
                    ket: `hb_kegiatan_${pIdx}_${kIdx}_ket`,
                },
            ...
            kegiatan.subKegiatan.forEach((subKegiatan, sIdx) => {
                ...
                const subKegiatanRow = createBudgetRow({
                    ...
                    inputNames: {
                        ...
                        ket: `hb_subkegiatan_${pIdx}_${kIdx}_${sIdx}_ket`,
                    },
...
function createBudgetRow(config) {
    ...
    const noPadding =
        config.level === "program"
            ? "0px"
            : config.level === "kegiatan"
            ? "20px"
            : "40px";
    const fontWeight =
        config.level === "program"
    ...
    nameTextarea.placeholder =
        ...
        (config.level === "program"
            ...
            ? "Kegiatan"
            : "Sub-Kegiatan");
    nameTextarea.style.cssText =
        "width: 95%; min-height: 35px; resize: none; padding: 4px; border: none; background: transparent; font-family: inherit; font-size: 12px;";
    ...
    amountInput.value =
        config.amount > 0
            ? parseInt(config.amount).toLocaleString("id-ID")
            : "0";
    amountInput.placeholder = "0";
    amountInput.className = "hierarchy-amount-input";
    amountInput.style.cssText =
        "width: 95%; border: none; background: transparent; text-align: right; font-family: inherit; font-size: 12px; font-weight: " +
        fontWeight;
...
function deleteSubKegiatan(pIdx, kIdx, sIdx) {
    if (confirm("Hapus sub-kegiatan ini?")) {
        const structure = getHierarchicalBudgetStructure();
        if (
            structure.programs[pIdx] &&
            structure.programs[pIdx].kegiatan[kIdx]
        ) {
...
function updateHierarchicalTotals() {
    ...
    rows.forEach((row) => {
        if (row.dataset.level === "subkegiatan") {
            ...
            if (amountInput && nameInput) {
                const match = nameInput.name.match(
                    /hb_subkegiatan_(\d+)_(\d+)_(\d+)/
                );
                if (match) {
                    const key = `${match[1]}_${match[2]}_${match[3]}`;
                    subKegiatanAmounts[key] = parseAmount(
                        amountInput.value || "0"
                    );
                }
            }
    ...
    if (grandTotalInput) {
        grandTotalInput.value =
            grandTotal > 0 ? grandTotal.toLocaleString("id-ID") : "0";
    }

    console.log("Hierarchical totals updated", {
        subKegiatanAmounts,
        kegiatanAmounts,
        programAmounts,
        grandTotal,
...
function saveToSupabase(e) {
    ...
    formData.append(
        "hierarchical_budget_json",
        JSON.stringify(hierarchicalStructure)
    );
    ...
    fetch("/perjanjian/save", {
        ...
        .then(async (response) => {
            ...
            if (contentType.includes("application/json")) {
                ...
                throw new Error(
                    "Unexpected server response: " + text.substring(0, 200)
                );
            }
